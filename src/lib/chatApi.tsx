export async function getConversations(userId: number) {
  const res = await fetch(`http://localhost:5291/api/chat/conversations/${userId}`);
...
export async function getMessages(conversationId: number, page = 1, pageSize = 50) {
  const res = await fetch(`http://localhost:5291/api/chat/messages/${conversationId}?page=${page}&pageSize=${pageSize}`);
