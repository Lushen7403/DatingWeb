...
const Dislikes = () => {
  const [dislikedUsers, setDislikedUsers] = useState<DislikedUser[]>([]);
      ...
      <div className="w-full max-w-md mx-auto p-4 relative z-10">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-2xl font-bold bg-gradient-to-r from-matchup-purple to-matchup-pink bg-clip-text text-transparent flex items-center gap-2">
            <X className="h-7 w-7 mr-1 text-matchup-pink animate-pulse" />
            Người bạn đã bỏ qua
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 h-[60vh]">
            <div className="w-24 h-24 rounded-full bg-muted flex items-center justify-center mb-4 shadow-lg">
              <X size={32} className="text-muted-foreground animate-bounce" />
            </div>
            <h3 className="text-lg font-semibold text-center mb-2">Chưa có ai bị bỏ qua</h3>
            <p className="text-muted-foreground text-center">
              Danh sách người bạn đã bỏ qua sẽ hiển thị tại đây
            </p>
          </div>
        ) : (
            ...
            {dislikedUsers.map((user) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <Avatar className="h-14 w-14 ring-2 ring-red-300 hover:scale-110 transition-transform shadow-md">
                    <AvatarImage src={user.avatar} alt={user.fullName} />
                    <AvatarFallback>{user.fullName.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div>
                    <h3 className="font-semibold text-lg text-matchup-purple">{user.fullName}</h3>
                    <p className="text-sm text-muted-foreground">Đã bị bỏ qua</p>
                  </div>
                </div>
              </Card>
            ))}
